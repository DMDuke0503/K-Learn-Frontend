...
const Topic = () => {
    ...
    const handleCountVocab = (topic_id) => {
        axios({
            method: "GET",
            url: `http://localhost:8080/api/vocabulary/count/${topic_id}`,
        })
        .then(res => {
            setVocabCounts(prevCounts => ({
                ...prevCounts,
                [topic_id]: res.data.count_vocabulary
            }));
        })
        .catch(err => {
            console.log(err);
    ...
    const handleTopic = async () => {
        try {
            const res = await axios({
                method: "GET",
                url: `http://localhost:8080/api/vocabulary_topic/course/${state.parent_course.id}`,
            });
            setTopics(res.data);

            console.log(topics);

            res.data.forEach(topic => handleCountVocab(topic.id));
        } catch (err) {
            console.log(err);
        }
    };
                                ...
                                <div className="w-full h-full flex flex-col justify-between">
                                    ...
                                    <div className="flex justify-between">
                                        <div className="flex items-center space-x-1">
                                            ...
                                            <p className="font-extrabold text-sm">{vocabCounts[topic.id] || 0} từ vựng</p>
                                        </div>
                                        <Lock size={25}></Lock>
                                    </div>
                                </div>
                            ...
                            <DialogContent className="h-4/5 overflow-y-scroll">
                                <div className="w-full h-full flex flex-col items-center p-5 space-y-5">
                                    <div className="flex flex-col items-center space-y-3">
                                        ...
                                        <p className="font-semibold">Từ và cụm từ: {vocabCounts[topic.id] || 0}</p>
                                        <Link 
